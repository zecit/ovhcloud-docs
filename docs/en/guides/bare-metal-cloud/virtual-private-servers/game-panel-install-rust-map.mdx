...
description: Install a custom Rust map on your game server by editing the server configuration file via SFTP and FileZilla.
...
Connect to your Rust server using SFTP. Refer to [Enable SFTP on the Game Panel](/guides/bare-metal-cloud/virtual-private-servers/game-panel-enable-sftp/) for setup instructions.
...
2. Locate the `server.cfg` file, right-click it and select <code className="action">Edit</code>.
